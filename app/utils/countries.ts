...
export function getAllCountries(): CountryOption[] {
    return allCountriesForSelection.map((country: CountryData) => ({
...
export function getCountryByName(countryName: string): CountryOption | undefined {
    const country = allCountriesForSelection.find((c: CountryData) =>
...
export function getPopularCountries(): CountryOption[] {
    const popularCodes = [
        'US', 'GB', 'CA', 'AU', 'FR', 'DE', 'ES', 'IT', 'NL', 'BE', 'AT', 'CH',
        'JP', 'CN', 'IN', 'BR', 'MX', 'SE', 'NO', 'DK', 'FI', 'SG', 'MY'
    ]
...
export function getAllCurrencies(): CurrencyOption[] {
    ...
    (countriesCodes as CountryData[]).forEach((country: CountryData) => {
        if (country.currencyCode && country.currencyName && !uniqueCurrencies.has(country.currencyCode)) {
...
export function getPopularCurrencies(): CurrencyOption[] {
    const popularCodes = [
        ...
        'MYR', 'THB', 'PHP', 'IDR', 'VND', 'KRW', 'BRL', 'MXN', 'ARS', 'CLP'
    ]
...
export function getCurrencyByCode(currencyCode: string): CurrencyOption | undefined {
    const country = (countriesCodes as CountryData[]).find((c: CountryData) => c.currencyCode === currencyCode)
    if (!country) return undefined
...
export function getEuropeanCountries(): CountryOption[] {
    return europeanCountries.map((country: CountryData) => ({
        value: country.isoCode,
        label: country.countryName,
        currencyCode: country.currencyCode,
        currencyName: country.currencyName,
