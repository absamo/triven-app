...
const ProductRow = ({ product, index }: { product: IProduct; index: number }) => {
    ...
    const status = stockQuantity === 0 ? 'Out of Stock' :
        stockQuantity <= (product.safetyStockLevel || 10) ? 'Low Stock' : 'In Stock';
...
const ProductListPDF = ({ products }: { products: IProduct[] }) => {
    ...
    const totalValue = products.reduce((sum, product) =>
        sum + ((product.physicalStockOnHand || 0) * Number(product.sellingPrice || 0)), 0
    );
    const lowStockCount = products.filter(product =>
        (product.physicalStockOnHand || 0) <= (product.safetyStockLevel || 10)
    ).length;
    const outOfStockCount = products.filter(product =>
        (product.physicalStockOnHand || 0) === 0
    ).length;
...
export const exportProductsToPDF = async (products: IProduct[], filename: string = 'products-report.pdf') => {
    try {
