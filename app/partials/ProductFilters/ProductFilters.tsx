...
export default function ProductFilters({
    ...
    useEffect(() => {
        ...
        const expectedParams = ['statuses', 'categories', 'agencies', 'sites', 'search', 'reorderAlert', 'deadStock', 'accuracyFilter', 'sortBy', 'sortOrder', 'limit', 'offset', 'agency']
        ...
        if (unexpectedParams.length > 0) {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.warn('[ProductFilters] Unexpected URL parameters detected:', unexpectedParams, 'Values:', unexpectedParams.map(key => ({ [key]: allParams[key] })))
            }
        }
        ...
        if (statusesParam || categoriesParam || agenciesParam || sitesParam || searchParam) {
            ...
            const searchToUse = searchParam ? (searchParam.startsWith('"') ? JSON.parse(searchParam) : searchParam) : null
            ...
            <Grid.Col span="auto">
                <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                    <div style={{
                        ...
                        <span style={{ fontSize: '14px', color: 'transparent' }}>.</span> {/* Invisible placeholder for alignment */}
                    </div>
