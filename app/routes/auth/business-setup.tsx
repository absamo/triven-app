...
export const loader: LoaderFunction = async ({ request }): Promise<{ user: any } | Response> => {
    ...
    if (!user && isVerified) {
        return redirect(`/login?verified=true&message=Email verified successfully! Please sign in to continue.${email ? `&email=${encodeURIComponent(email)}` : ''}`);
    }
...
export const action: ActionFunction = async ({ request }): Promise<{ error?: string } | Response> => {
    const user = await getBetterAuthUser(request)
...
export default function BusinessSetupRoute({ loaderData, actionData }: { loaderData: { user: any }, actionData: { error?: string } }) {
