    ...
    if (result && result.user && result.status) {
      ...
      if (inviteToken) {
        try {
          ...
          if (
            invitation &&
            invitation.status === INVITATION_STATUSES.INVITED &&
      ...
      return {
        error:
          'The verification code you entered is incorrect or has expired. Please check your code or request a new one.',
        email,
      }
    }
  } catch (error: any) {
