...
import BackorderCreate from '~/app/pages/Backorders/BackorderCreate'
import { getAgencies } from '~/app/services/agencies.server'
import { createBackorder, getMaxBackorderNumber } from '~/app/services/backorders.server'
...
export const loader: LoaderFunction = async ({ request }: LoaderFunctionArgs) => {
  ...
  const backorder = {
    backorderReference: `BO-${backorderNumber}`,
    customerId: '',
    agencyId: '',
    siteId: '',
    originalOrderDate: new Date(),
    notes: '',
    backorderItems: [],
  }
...
export const action: ActionFunction = async ({ request }: ActionFunctionArgs) => {
  ...
  return await createBackorder(request, backorderData)
}

export default function BackorderCreateRoute({ loaderData, actionData }: any) {
