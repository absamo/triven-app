...
import { deleteProduct, duplicateProduct, getFilteredProducts, getProducts, getProductsForAgency } from "~/app/services/products.server"
...
export const loader: LoaderFunction = async ({
  ...
  try {
    ...
    const search = searchParam ? (searchParam.startsWith('"') ? JSON.parse(searchParam) : searchParam) : undefined
    ...
    const hasFilters = (statuses && statuses.length > 0) || (categoryFilters && categoryFilters.length > 0) || (agencyFilters && agencyFilters.length > 0) || (siteFilters && siteFilters.length > 0) || search || reorderAlert || deadStock || accuracyFilter
    ...
    if (agencyParam && agencyParam !== "All") {
      ...
      if (deadStock || hasFilters) {
        ...
        products = (allFilteredProducts as any[]).filter(product =>
          product.agencyId === agencyParam
        ) as any as IProduct[]
      } else {
...
export default function ProductsRoute({ loaderData, actionData }: Route.ComponentProps) {
  const { products, categories, agencies, sites, permissions, isFiltered, userRole } = loaderData as unknown as {
    products: IProduct[]
