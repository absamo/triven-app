...
export const action: ActionFunction = async ({
  ...
  const sellingPrice = JSON.parse(formData.get("sellingPrice") as string) as IProduct["sellingPrice"]
  const barcode = formData.get("barcode") as IProduct["barcode"]
  const reorderPoint = JSON.parse(formData.get("reorderPoint") as string) as IProduct["reorderPoint"]
  const safetyStockLevel = JSON.parse(formData.get("safetyStockLevel") as string) as IProduct["safetyStockLevel"]
  ...
  const openingStock = JSON.parse(formData.get("openingStock") as string) as IProduct["openingStock"]
  const openingValue = JSON.parse(formData.get("openingValue") as string) as IProduct["openingValue"]
  const adjustedQuantity = JSON.parse(formData.get("adjustedQuantity") as string) as IProduct["adjustedQuantity"]
  const availableQuantity = JSON.parse(formData.get("availableQuantity") as string) as IProduct["availableQuantity"]
    ...
    if (file && file.size > 0) {
      try {
        ...
        if (uploadResult.success && uploadResult.data) {
          ...
          const isPrimary: boolean = formData.get(`imagePrimary_${i}`) === 'true' || (i === 0 && uploadedImages.length === 0) // First image is primary by default
...
export default function CreateProductsRoute({
  loaderData,
  actionData,
