    ...
    const stream = new ReadableStream({
      async start(controller) {
        try {
          ...
          const streamResult = await agent.stream(messages, {
            ...
            onChunk: async (chunk: any) => {
              ...
              if (chunk.type === 'tool-call') {
                console.log('🔧 Tool called:', chunk.payload?.toolName || chunk.payload?.name || 'unknown')
              }
              
              // Handle tool results
              if (chunk.type === 'tool-result') {
                console.log('🔧 Tool result:', chunk.payload.toolName, chunk.payload.result)
                const toolOutput = formatToolOutput(chunk.payload.toolName, chunk.payload.result, language)
                if (toolOutput) {
              ...
              if (uiMessage.parts && Array.isArray(uiMessage.parts)) {
                ...
                for (const part of uiMessage.parts) {
                  ...
                  if (partAny.type && partAny.type.startsWith('tool-') && partAny.output) {
                    ...
                    if (formattedOutput) {
                      console.log('� Sending formatted tool output')
                      
                      // Send the formatted tool output to the frontend
                      controller.enqueue(encoder.encode('event: message\n'))
                      controller.enqueue(encoder.encode(`data: ${JSON.stringify({ content: formattedOutput })}\n\n`))
                    }
                  }
                }
              }
          ...
          if (finalResult.text && typeof finalResult.text === 'string' && finalResult.text.includes('|')) {
