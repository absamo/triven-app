...
export const action: ActionFunction = async ({ request }) => {
    ...
    try {
        ...
        if (!headers.includes('name')) {
            return new Response(
                JSON.stringify({ error: 'CSV file must contain a "name" column' }),
                {
                    status: 400,
                    headers: { 'Content-Type': 'application/json' },
                }
        ...
        for (let i = 1; i < lines.length; i++) {
            ...
            try {
                ...
                const categoryForResponse = {
                    id: (createdCategory && typeof createdCategory === 'object' && 'id' in createdCategory) ? createdCategory.id : undefined,
