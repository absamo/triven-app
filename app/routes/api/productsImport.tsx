            ...
            try {
                ...
                if (row.sellingPrice) {
                    ...
                    if (isNaN(priceValue) || priceValue <= 0) {
                        rowErrors.push(`Invalid selling price "${row.sellingPrice}" (must be a positive number)`);
                    }
                }
                ...
                if (row.category && row.category.trim()) {
                    if (categories && categories.length > 0) {
                        ...
                        if (!category) {
                            ...
                            rowErrors.push(`Category "${row.category}" does not exist.\nAvailable categories:${availableCategoriesList}`);
                        } else {
                            categoryId = category.id;
                        }
                    } else {
                        // Category specified but no categories exist in the system
                        rowErrors.push(`Category "${row.category}" specified but no categories exist in your system. Please create categories first.`);
                    }
                } else {
                    ...
                    if (categories && categories.length > 0) {
                        ...
                        rowErrors.push(`No category specified and no categories exist in your system. Please create at least one category first.`);
                    }
                }
                ...
                const productData = {
                    ...
                    openingValue: (parseFloat(row.sellingPrice) || 0) * (parseInt(row.physicalStockOnHand) || 0),
                    siteId: userSite?.id || '',
                    agencyId: userAgency?.id || '',
                    unit: mapProductUnit(row.unitOfMeasure) as "Pieces" | "Box" | "Pack" | "Dozen" | "Kilogram" | "Pound" | "Meter" | "Centimeter" | "Liters" | "Milliliter" | "Gram" | "Milligram" | "Ounce" | "Foot" | "Inch",
                };
                ...
                const productForResponse = {
                    id: (createdProduct && typeof createdProduct === 'object' && 'id' in createdProduct) ? createdProduct.id : undefined,
