...
export const inventoryTools = [
    {
        ...
        function: {
            ...
            description: 'Get current inventory statistics including total products, low stock alerts, and value',
            parameters: {
                type: 'object',
                properties: {},
                required: []
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Get a comprehensive list of all products in the inventory with their details. Use this tool when user asks for: "show me all products", "list all products", "display all products", "what products do you have", "view all inventory", "liste moi les produits", "liste les produits", "montre moi les produits", "affiche tous les produits", "voir tous les produits", or similar requests.',
            parameters: {
                type: 'object',
                properties: {
    ...
    },
    {
        ...
        function: {
            ...
            description: 'Navigate user to a specific page in the application. Use this when user requests to go to, view, or navigate to a specific section of the app.',
            parameters: {
                type: 'object',
                properties: {
                    page: {
                        type: 'string',
                        description: 'The page to navigate to. Available pages include main sections and management areas.',
                        enum: [
                            'home', 'dashboard', 'products', 'chat', 'customers', 'suppliers',
                            'categories', 'sales-orders', 'purchase-orders', 'invoices',
                            'bills', 'stock-adjustments', 'analytics', 'settings', 'sites',
                            'teams', 'roles', 'agencies', 'payments-made', 'payments-received',
                            'transfer-orders'
                        ]
                    },
                    reason: {
    ...
    switch (name) {
        case 'get_inventory_stats':
            ...
            const lowStockProducts = productData.filter(p =>
                p.safetyStockLevel && p.availableQuantity <= p.safetyStockLevel
            ).length;
        ...
        case 'check_stock_level':
            ...
            if (!product) {
                ...
                const suggestions = actualProducts.length > 0
            ...
            return {
                ...
                status: product.availableQuantity === 0 ? 'Out of Stock' :
                    product.safetyStockLevel && product.availableQuantity <= product.safetyStockLevel ? 'Low Stock' :
                        'Good Stock',
        ...
        case 'get_low_stock_products':
            ...
            const filteredLowStock = allProducts.filter(item =>
        ...
        case 'calculate_reorder_suggestion':
            ...
            const dailySales = reorderProduct.stockAdjustmentHistories.length > 0
                ? reorderProduct.stockAdjustmentHistories.reduce((sum: number, adjustment: any) =>
                    sum + Math.abs(adjustment.adjustedQuantity), 0) / 30
            ...
            const reorderPoint = reorderProduct.reorderPoint || Math.ceil((dailySales * leadTimeDays) + safetyStock);
            ...
            return {
                ...
                urgency: reorderProduct.availableQuantity <= reorderPoint ? 'High' :
                    reorderProduct.availableQuantity <= (reorderPoint * 1.5) ? 'Medium' : 'Low'
            };
        ...
        case 'get_product_details':
            ...
            if (!searchProduct) {
                ...
                const fallbackSuggestion = sampleProducts.length > 0
            ...
            return {
                product_details: {
                    basic_info: {
                        ...
                        suppliers: searchProduct.suppliers.map(s => s.name).join(', ') || 'No suppliers assigned',
                    ...
                    stock_info: {
                        ...
                        status: searchProduct.availableQuantity === 0 ? 'Out of Stock' :
                            searchProduct.safetyStockLevel && searchProduct.availableQuantity <= searchProduct.safetyStockLevel ? 'Low Stock' :
                                'Normal'
                    },
                    pricing_info: {
