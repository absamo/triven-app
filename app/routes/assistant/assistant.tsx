...
export function meta({}: Route.MetaArgs) {
  return [
    ...
    {
      name: 'description',
      content: 'AI assistant for inventory management and general conversation',
    },
  ]
}
...
const MarkdownComponents = {
  ...
  blockquote: ({ children, ...props }: any) => (
    <Box pl="md" mb="sm" className={classes.markdownBlockquote} {...props}>
  ...
  code: ({ children, className, ...props }: any) => {
    ...
    if (isInline) {
      return (
        <Text span className={classes.markdownInlineCode} {...props}>
          {children}
        </Text>
      )
    }
    ...
    return (
      <Box p="md" mb="sm" className={classes.markdownCodeBlock} {...props}>
        <Text className={classes.markdownCodeText}>{children}</Text>
      </Box>
    )
  },
  ...
  tbody: ({ children, ...props }: any) => <Table.Tbody {...props}>{children}</Table.Tbody>,
  tr: ({ children, ...props }: any) => <Table.Tr {...props}>{children}</Table.Tr>,
  ...
  hr: ({ ...props }: any) => <Divider my="lg" className={classes.markdownHr} {...props} />,
}
...
export default function Chat() {
  ...
  // Streaming function
  const startStreamingResponse = async (message: string) => {
    setIsStreaming(true)
    ...
    try {
      ...
      const eventSource = new EventSource(`/ai-assistant/stream?${params}`)
      eventSourceRef.current = eventSource

      // Listen for 'chunk' events (our main content stream)
        ...
        setMessages((prev) =>
          prev.map((msg) =>
            msg.id === assistantMessageId
              ? {
                  ...msg,
                  content: msg.content || t('assistant:failedToGetResponse'),
                  isStreaming: false,
                }
              : msg
          )
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.id === assistantMessageId
            ? {
                ...msg,
                content: t('assistant:failedToStartStreaming'),
                isStreaming: false,
              }
            : msg
        )
  ...
  return (
    <Box className={classes.chatContainer}>
      {messages.length === 0 ? (
            ...
            <Stack align="center" gap="xl">
              <Box ta="center">
                ...
                <Text size="xl" fw={600} mb="xs">
                  {currentHour < 12
                    ? t('assistant:goodMorning')
                    : currentHour < 18
                      ? t('assistant:goodAfternoon')
                      : t('assistant:goodEvening')}
                </Text>
        ...
        /* Messages */
        <ScrollArea ref={scrollRef} className={classes.messagesScrollArea}>
          <Stack gap="lg">
                ...
                <Box key={message.id}>
                  ...
                  <Group align="flex-start" gap="md">
                    <Avatar
                      ...
                      src={
                        message.role === 'user'
                          ? user.image || user.profile?.avatar || undefined
                          : undefined
                      }
                      name={
                        message.role === 'user'
                    ...
                    <Box flex={1} maw="100%">
                      {message.role === 'assistant' ? (
                        <Box>
                          ...
                          >
                            {message.content ||
                              (message.isStreaming ? '' : t('assistant:noResponseGenerated'))}
                          </ReactMarkdown>
