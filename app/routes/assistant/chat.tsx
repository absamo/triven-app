...
import {
  ...
  getRecentAnomalies,
} from '~/app/services/ai-anomaly-detection.server'
    ...
    switch (name) {
      ...
      case 'get_reorder_recommendations': {
        ...
        try {
          const recommendations =
            await forecastingService.generateAutoReorderRecommendations(companyId)
      ...
      case 'analyze_product_performance': {
        ...
        if (!productIdForAnalysis) {
          ...
          const productsWithSales = recentProducts
            .map((product) => ({
              id: product.id,
              name: product.name,
              sku: product.sku,
          ...
          return {
            error:
              "Please specify which product you'd like me to analyze. Here are some suggestions:",
            ...
            message:
              'Try asking: "Analyze product performance for [product name]" or specify a product ID.',
          }
        }
...
**Key Factors:**
${
  actualResult.factors
    ? Object.entries(actualResult.factors)
...
${
  actualResult.recommendations?.length
...
${
  actualResult.anomalies?.length
    ? actualResult.anomalies
        .map(
...
When discussing inventory, always use real data from the database. Present information naturally and format it for easy reading.`,
      ...
      tools: useTools
        ? {
            ...
            get_products: {
              description:
                'Get a list of products from the inventory database. Use a high limit (like 1000) when user asks for "all products" or "show me all products". Supports both English and French language.',
              inputSchema: z.object({
                limit: z
                  .number()
                  .optional()
                  .describe(
                    'Maximum number of products to return. Use 1000 for "all products" requests, default: 10 for general requests'
                  ),
                language: z
                  .enum(['en', 'fr'])
                  .optional()
                  .describe(
                    'Language for response - "en" for English, "fr" for French. Detect from user message.'
                  ),
                time_period: z
                  .string()
                  .optional()
                  .describe(
                    'Time period for filtering data (e.g., "today", "last week", "last month", "last quarter", "7 days", "30 days")'
                  ),
              }),
              execute: async (args) => {
            ...
            generate_demand_forecast: {
              description:
                'Generate AI-powered demand forecast for a product using historical sales data',
              inputSchema: z.object({
                product_name: z.string().optional().describe('Name of the product to forecast'),
                product_id: z
                  .string()
                  .optional()
                  .describe('ID of the product to forecast (if known)'),
                forecast_days: z
                  .number()
                  .optional()
                ...
                time_period: z
                  ...
                  .optional()
                  .describe(
                    'Time period for historical data analysis (e.g., "today", "last week", "last month", "last quarter", "7 days", "30 days")'
                  ),
              }),
              execute: async (args) => {
                try {
            ...
            get_reorder_recommendations: {
              description:
                'Get AI-generated purchase order recommendations for products that need restocking',
              inputSchema: z.object({
                urgency_filter: z
            ...
            run_anomaly_scan: {
              description:
                'Run AI-powered anomaly detection on current inventory data to identify unusual patterns or issues',
              inputSchema: z.object({
                scope: z
                  .enum(['inventory', 'sales', 'suppliers', 'all'])
                  .optional()
                  .describe('Scope of anomaly detection'),
                time_period: z
                  .string()
                  .optional()
                  .describe(
                    'Time period for anomaly detection (e.g., "today", "last week", "last month", "last quarter", "7 days", "30 days")'
                  ),
              }),
              execute: async (args) => {
                try {
            ...
            get_recent_anomalies: {
              ...
              inputSchema: z.object({
                limit: z
                  .number()
                  .optional()
                  .describe('Number of anomalies to return (default: 10)'),
                time_period: z
                  .string()
                  .optional()
                  .describe(
                    'Time period for fetching anomalies (e.g., "today", "last week", "last month", "last quarter", "7 days", "30 days")'
                  ),
              }),
              execute: async (args) => {
            ...
            generate_business_insights: {
              description:
                'Generate AI-powered business insights and recommendations based on current data',
              inputSchema: z.object({
                focus_area: z
                ...
                time_period: z
                  ...
                  .optional()
                  .describe(
                    'Time period for data analysis (e.g., "today", "last week", "last month", "last quarter", "7 days", "30 days")'
                  ),
              }),
              execute: async (args) => {
            ...
            analyze_product_performance: {
              description:
                'Analyze a SPECIFIC product by name or ID with detailed insights including sales, profitability, and inventory turnover. Use this when the user mentions a specific product name.',
              inputSchema: z.object({
                ...
                time_period: z
                  ...
                  .optional()
                  .describe(
                    'Time period for performance analysis (e.g., "today", "last week", "last month", "last quarter", "7 days", "30 days")'
                  ),
              }),
              execute: async (args) => {
                return await executeToolFunction(
            ...
            analyze_overall_product_performance: {
              description:
                'Analyze overall product performance across ALL products when no specific product is mentioned. Use this for general product performance queries like "analyze product performance", "how are products performing", "product performance overview".',
              inputSchema: z.object({
                time_period: z
                  .string()
                  .optional()
            ...
            get_optimization_recommendations: {
              description:
                'Get AI-powered optimization recommendations for inventory management based on specific goals',
              inputSchema: z.object({
                optimization_goal: z
                ...
                time_period: z
                  ...
                  .optional()
                  .describe(
                    'Time period for optimization analysis (e.g., "today", "last week", "last month", "last quarter", "7 days", "30 days")'
                  ),
              }),
              execute: async (args) => {
                return await executeToolFunction(
            ...
            explain_kpi_trends: {
              description:
                'Explain trends in key performance indicators with AI analysis and recommendations',
              inputSchema: z.object({
                kpi_name: z
                  .string()
                  .optional()
                  .describe(
                    'Name of the KPI to analyze (e.g., "inventory turnover", "gross margin")'
                  ),
                time_period: z
                  .string()
                  .optional()
                  .describe(
                    'Time period for trend analysis (e.g., "today", "last week", "last month", "last quarter", "7 days", "30 days", default: "last 30 days")'
                  ),
              }),
              execute: async (args) => {
            ...
            natural_language_query: {
              description:
                'Process natural language questions about business data and provide insights',
              inputSchema: z.object({
            ...
            show_low_stock_items: {
              description:
                'Show items that are low in stock, out of stock, or critically low. Use this specifically when user asks to "show low stock", "list low stock items", "out of stock items", etc.',
              inputSchema: z.object({
                time_period: z
                  .string()
                  .optional()
                  .describe(
                    'Time period for stock analysis (e.g., "today", "last week", "last month", "last quarter", "7 days", "30 days")'
                  ),
              }),
              execute: async (args) => {
