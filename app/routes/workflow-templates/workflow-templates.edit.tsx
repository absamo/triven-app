...
export async function loader({ params, request }: LoaderFunctionArgs) {
    ...
    try {
        ...
        const transformedTemplate = {
            ...
            entityType: workflowTemplate.triggerType.includes('purchase_order') ? 'purchase_order' :
                workflowTemplate.triggerType.includes('sales_order') ? 'sales_order' :
                    workflowTemplate.triggerType.includes('stock_adjustment') ? 'stock_adjustment' :
                        workflowTemplate.triggerType.includes('transfer_order') ? 'transfer_order' :
                            workflowTemplate.triggerType.includes('invoice') ? 'invoice' :
                                workflowTemplate.triggerType.includes('bill') ? 'bill' :
                                    workflowTemplate.triggerType.includes('customer') ? 'customer' :
                                        workflowTemplate.triggerType.includes('supplier') ? 'supplier' :
                                            workflowTemplate.triggerType.includes('product') ? 'product' :
                                                'custom',
