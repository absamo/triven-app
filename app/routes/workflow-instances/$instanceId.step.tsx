...
export async function loader({ request, params }: LoaderFunctionArgs) {
    ...
    const currentStepExecution = workflowInstance.stepExecutions.find(
        exec => exec.workflowStep.stepNumber === workflowInstance.currentStepNumber &&
    ...
    if (currentStep) {
        if (currentStep.assigneeType === 'user' && currentStep.assigneeUserId === session.user.id) {
            canApprove = true
        } else if (currentStep.assigneeType === 'role' && session.user.roleId === currentStep.assigneeRoleId) {
            canApprove = true
        }
    }
