    ...
    try {
        ...
        const detailedProducts = products.map(product => {
            ...
            const qtyOrdered = product.purchaseOrderItems
                .filter(item => item.purchaseOrder && item.purchaseOrder.status !== PURCHASE_ORDER_STATUSES.CANCELLED)
                .reduce((sum, item) => sum + item.quantity, 0);

            // Calculate quantities received (from purchase receives)
            const qtyIn = product.purchaseReceiveItems
            ...
            const qtyOut = product.salesOrderItems
                .filter(item => item.salesOrder && item.salesOrder.status !== SALES_ORDERS_STATUSES.CANCELLED)
                .reduce((sum, item) => sum + item.quantity, 0);
        ...
        const lowStockItems = productsWithCalculatedStatus.filter(p =>
            p.calculatedStatus === 'LowStock'
        ).length

        const outOfStockItems = productsWithCalculatedStatus.filter(p =>
            p.calculatedStatus === 'OutOfStock'
        ).length
