...
export async function executeNextStep(workflowInstanceId: string) {
    ...
    if (!instance || (instance.status !== WORKFLOW_STATUSES.PENDING && instance.status !== WORKFLOW_STATUSES.IN_PROGRESS)) {
        return null
    }
        ...
        if (existingApprovalRequest) {
            ...
            await prisma.approvalRequest.update({
                ...
                data: {
                    ...
                    title: currentStep.stepNumber > 1
    ...
    if (request.workflowInstance) {
        ...
        if (data.status === 'reopen') {
            ...
            if (activeStepExecution) {
                ...
                console.warn(`No workflow step execution found for approval request ${requestId} (workflowInstance: ${request.workflowInstance.id})`)
            }
        }
    }
