...
export async function getFilteredBackordersByStatus(request: Request, statuses: string[]) {
  const user = await requireBetterAuthUser(request, ['read:backorders'])
...
export async function getBackorder(request: Request, backorderId: IBackorder['id']) {
  const user = await requireBetterAuthUser(request, ['read:backorders'])
...
export async function createBackorder(request: Request, backorder: IBackorder) {
...
export async function updateBackorder(request: Request, backorder: IBackorder) {
  ...
  if (foundBackorder) {
    return {
      errors: {
        backorderReference: 'A backorder already exists with this reference',
      },
    }
  }
...
export async function updateBackorderStatus(
  request: Request,
  { backorderId, status }: { backorderId: IBackorder['id']; status: IBackorder['status'] }
) {
  const user = await requireBetterAuthUser(request, ['update:backorders'])
...
export async function fulfillBackorderItem(
  request: Request,
  { backorderItemId, fulfilledQuantity }: { backorderItemId: string; fulfilledQuantity: number }
) {
  const user = await requireBetterAuthUser(request, ['update:backorders'])
  ...
  try {
    ...
    if (updatedBackorder) {
      ...
      const someItemsFulfilled = updatedBackorder.backorderItems.some(
        (item) =>
          item.status === BACKORDER_ITEM_STATUSES.FULFILLED ||
