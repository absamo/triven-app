...
export class DemandForecastingService {
    ...
    private async getHistoricalSalesData(productId: string) {
        ...
        return {
            ...
            salesHistory: salesHistory.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()),
            totalSales: salesHistory.reduce((sum, entry) => sum + entry.quantity, 0),
            averageDailySales: salesHistory.length > 0 ? salesHistory.reduce((sum, entry) => sum + entry.quantity, 0) / salesHistory.length : 0
        };
    }
...
${historicalData.salesHistory.map((entry: any) =>
...
Be realistic and conservative in your predictions. If historical data is limited, indicate lower confidence.
    ...
    async updateForecastAccuracy() {
        ...
        for (const forecast of outdatedForecasts) {
            try {
                ...
                const normalizedError = forecast.predictedDemand > 0 ? error / forecast.predictedDemand : (actualSales > 0 ? 1 : 0);
        ...
        for (const product of products) {
            try {
                ...
                if (forecast && product.suppliers && product.suppliers.length > 0) {
                    ...
                    if (reorderQty > 0) {
                        const urgency = this.calculateUrgency(product.availableQuantity, forecast.predictedDemand);
    ...
    private calculateUrgency(currentStock: number, predictedDemand: number): 'Low' | 'Medium' | 'High' | 'Critical' {
