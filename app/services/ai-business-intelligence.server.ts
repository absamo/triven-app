...
export class BusinessIntelligenceService {
    ...
    async generateBusinessInsights(companyId: string, focusArea?: string, timeRange?: { startDate: Date; endDate: Date; description: string }) {
        try {
...
Reference the specific time period analyzed in your insights.
    ...
    async processNaturalLanguageQuery(query: string, companyId: string, timeRange?: { startDate: Date; endDate: Date; description: string }) {
        try {
            // Check if this is specifically a low stock query
            const isLowStockQuery = query.toLowerCase().includes('low stock') ||
...
Format the response in markdown for better readability.
            `.trim(); const response = await this.ollama.chat({
                model: 'llama3.1:8b',
                messages: [{ role: 'user', content: prompt }]
            });
    ...
    async getLowStockItemsOnly(companyId: string, timeRange?: { startDate: Date; endDate: Date; description: string }) {
        try {
    ...
    private async getBusinessDataSummary(companyId: string, timeRange?: { startDate: Date; endDate: Date; description: string }) {
        ...
        const lowStockProducts = productsWithCalculatedStatus.filter(p =>
            p.calculatedStatus === PRODUCT_STATUSES.LOWSTOCK
        ).map(p => ({
            id: p.id,
            name: p.name,
            sku: p.sku || '',
        ...
        const outOfStockProducts = productsWithCalculatedStatus.filter(p =>
            p.calculatedStatus === PRODUCT_STATUSES.OUTOFSTOCK
        ).map(p => ({
            id: p.id,
            name: p.name,
            sku: p.sku || '',
        ...
        const criticalStockProducts = productsWithCalculatedStatus.filter(p =>
            p.calculatedStatus === PRODUCT_STATUSES.CRITICAL
        ).map(p => ({
            id: p.id,
            name: p.name,
            sku: p.sku || '',
        ...
        const totalRevenue = salesOrders.reduce((sum, order) =>
            sum + order.salesOrderItems.reduce((itemSum, item) => itemSum + item.amount, 0), 0
        );
        const totalOrders = salesOrders.length;
        const averageOrderValue = totalOrders > 0 ? totalRevenue / totalOrders : 0;

        // Product performance
        const productSales: Record<string, { quantity: number; revenue: number; name: string; sku: string }> = {};
        ...
        const totalCostOfGoodsSold = salesOrders.reduce((sum, order) =>
            sum + order.salesOrderItems.reduce((itemSum, item) => {
        ...
        const inventoryTurnover = averageInventoryValue > 0 ? (totalCostOfGoodsSold / averageInventoryValue) * 12 : 0; // Annualized

        // Stock coverage days
        const totalDemand = products.reduce((sum, p) =>
            sum + p.salesOrderItems.reduce((itemSum, item) => itemSum + item.quantity, 0), 0
        );
        const analysisDays = Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
        ...
        const firstHalfSales = salesOrders
            .filter(order => order.orderDate >= startDate &&
                order.orderDate < midDate)
            .reduce((sum, order) => sum + order.salesOrderItems.reduce((itemSum, item) => itemSum + item.amount, 0), 0);

        const secondHalfSales = salesOrders
            .filter(order => order.orderDate >= midDate &&
    ...
    async analyzeOverallProductPerformance(companyId: string, timeRange?: { startDate: Date; endDate: Date; description: string }) {
        try {
            // Default to 30 days if no time range provided
            const defaultStartDate = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
            const startDate = timeRange?.startDate || defaultStartDate;
            ...
            const productPerformance = products.map(product => {
                const totalQuantitySold = product.salesOrderItems.reduce((sum, item) => sum + item.quantity, 0);
                const totalRevenue = product.salesOrderItems.reduce((sum, item) => sum + item.amount, 0);
                ...
                return {
                    ...
                    totalRevenue,
                    profitMargin,
                    stockStatus: product.availableQuantity <= (product.reorderPoint || 0) ? 'Low Stock' : 'Normal',
            ...
            const summaryData = {
                ...
                averageMargin: productPerformance.length > 0 ?
                    productPerformance.reduce((sum, p) => sum + p.profitMargin, 0) / productPerformance.length : 0,
...
Include context about the time period analyzed (${timeDescription}).
      `.trim();
            ...
            throw new Error(`Failed to analyze overall product performance: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }

    async analyzeProductPerformance(productId: string, analysisType: string = 'comprehensive', timeRange?: { startDate: Date; endDate: Date; description: string }) {
        try {
            // Default to 30 days if no time range provided
            const defaultStartDate = new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
            const startDate = timeRange?.startDate || defaultStartDate;
            ...
            const productData = {
                ...
                margin: ((product.sellingPrice - product.costPrice) / product.sellingPrice * 100).toFixed(2),
...
Include context about the time period analyzed.
      `.trim();
            ...
            throw new Error(`Failed to analyze product performance: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }

    async getOptimizationRecommendations(companyId: string, goal: string, timeRange?: { startDate: Date; endDate: Date; description: string }) {
        try {
            const businessData = await this.getBusinessDataSummary(companyId, timeRange);
...
Format as markdown with clear sections and bullet points.
    ...
    async explainKPITrends(companyId: string, kpiName: string, timeRange: { startDate: Date; endDate: Date; description: string }) {
        try {
            const businessData = await this.getBusinessDataSummary(companyId, timeRange);
...
Reference the specific time period (${timeRange.description}) in your analysis.
      `.trim();
            ...
            throw new Error(`Failed to explain KPI trends: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }

    private async getHistoricalKPIData(companyId: string, kpiName: string, timeRange: { startDate: Date; endDate: Date; description: string }) {
    ...
    private async calculateInventoryTurnoverHistory(companyId: string, startDate: Date, endDate: Date) {
        ...
        const averageInventoryValue = products.reduce((sum, p) =>
            sum + (p.availableQuantity * p.costPrice), 0
        );

        const days = Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24));
        const annualizedTurnover = averageInventoryValue > 0 ? (totalCOGS / averageInventoryValue) * (365 / days) : 0;
    ...
    async generateDailyInsights(companyId: string) {
        try {
            ...
            throw new Error(`Failed to generate daily insights: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    }
