...
async function getRecentSalesPatterns(companyId: string) {
    ...
    const productTotals: Record<string, { quantity: number; revenue: number; name: string; sku: string }> = {};
...
async function getSupplierPerformance(companyId: string) {
    ...
    purchaseOrders.forEach(order => {
        ...
        order.purchaseReceives.forEach(receive => {
            ...
            if (expectedDate && receivedDate) {
                const delayDays = Math.ceil((receivedDate.getTime() - expectedDate.getTime()) / (1000 * 60 * 60 * 24));
    ...
    return Object.entries(supplierMetrics).map(([id, metrics]) => ({
        id,
        ...metrics,
        onTimePercentage: metrics.totalOrders > 0 ?
            (metrics.onTimeDeliveries / (metrics.onTimeDeliveries + metrics.lateDeliveries)) * 100 : 0
    }));
}
...
async function getInventoryHealthMetrics(companyId: string) {
    ...
    const metrics = {
        ...
        lowStock: products.filter(p => p.availableQuantity > 0 && p.availableQuantity <= (p.reorderPoint || 5)).length,
        ...
        averageStockLevel: products.length > 0 ?
            products.reduce((sum, p) => sum + p.availableQuantity, 0) / products.length : 0,
...
function parseAnomalyResponse(response: string) {
    try {
        ...
        parsed.anomalies.forEach((anomaly: any, index: number) => {
            if (!anomaly.type || !anomaly.severity || !anomaly.description ||
