...
export default function Currency({
  ...
  const defaultCurrencies = currencies
    .reduce((acc: ICurrency[], currency: ICurrency) => {
      const found = companyCurrencies.find(
        (cc) => cc.isoCode === currency.isoCode
      ) as ICurrency
  ...
  const otherCurrencies = currencies.filter((c) => {
    return companyCurrencies.every(
      (companycurrency) => companycurrency.isoCode !== c.isoCode
    )
  })
  ...
  const SelectOption = ({ currencyCode, countryName, flag, placeholder: customPlaceholder }: Partial<CurrencyProps> & { placeholder?: string }) => {
    ...
    return (
      <Flex align="center" gap="sm">
        <ThemeIcon variant="transparent" size="sm">{flag}</ThemeIcon>
  ...
  const currencyOptions = (options: CurrencyProps[]) =>
    options
      ...
      .map((item, index) => (
        <Combobox.Option
          value={item.isoCode as string}
          key={index}
          active={item.isoCode === value}
        >
          <Group justify="space-between">
            <Flex align="center" gap="sm">
              <ThemeIcon variant="transparent" size="sm">{item.flag}</ThemeIcon>
              <div>
                <Text fz="sm" fw={500}>
                  {item.currencyCode}
                </Text>
      ...
      <Combobox
        ...
        onOptionSubmit={(currency) => {
          ...
          if (restrictToCompanyCurrencies) {
            currentCurrency = defaultCurrencies.find(
              (c) => c.isoCode === currency
            )
          } else {
        ...
        <Combobox.Dropdown>
          ...
          <Combobox.Options>
            <ScrollArea.Autosize type="scroll" mah={250}>
              <Combobox.Group
                label={
                  restrictToCompanyCurrencies ? null : t('defaultCurrencies')
                }
              >
              ...
              {!restrictToCompanyCurrencies && (
                <Combobox.Group
                  label={
                    defaultCurrencies.length > 0 ? t('otherCurrencies') : null
                  }
                >
