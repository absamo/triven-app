...
export default function ProductImportModal({ opened, onClose, onImport }: ProductImportModalProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/productsImport', {
        method: 'POST',
        body: formData,
      })
  ...
  // Check if modal should be closable (only blocked during import)
  const isModalClosable = !isImporting
    ...
    >
          ...
          <Stack gap="md" mt="md">
            ...
            {file && (
              <Alert
                ...
                title={t('inventory:fileSelected')}
                color="green"
              >
        ...
        >
          <Stack gap="md" mt="md">
            ...
            <Alert
              ...
              title={t('inventory:requiredFields')}
              color="blue"
            >
              <List size="sm">
                <List.Item>{t('inventory:productNamesMustBeUnique')}</List.Item>
                <List.Item>{t('inventory:categoryMustExist')}</List.Item>
                <List.Item>{t('inventory:sellingPriceMustBePositive')}</List.Item>
              </List>
            </Alert>

            <Alert
              icon={<IconAlertCircle size={16} />}
              title={t('inventory:validationRules')}
              color="yellow"
            >
              <List size="sm">
                <List.Item>{t('inventory:productNamesMustBeUnique')}</List.Item>
                <List.Item>{t('inventory:categoryValidationRule')}</List.Item>
                <List.Item>{t('inventory:duplicateProductNamesRejected')}</List.Item>
          ...
          <Stack gap="md" mt="md">
            ...
            {file && (
              <Alert
                ...
                title={t('inventory:importSummary')}
                color="blue"
              >
                <Text size="sm">
                  {t('inventory:fileName')}: {file.name}
                  <br />
            ...
            {isImporting && (
              <Stack gap="sm">
                ...
                <Text size="xs" c="dimmed">
                  {Math.round(importProgress)}% {t('inventory:complete')}
                </Text>
              </Stack>
            )}
          </Stack>
            ...
            {importResults && (
              <>
                ...
                (!importResults.warnings || importResults.warnings.length === 0) ? (
                  ...
                  >
                    <Text size="sm" mb="xs">
                      {t('inventory:productsImported', { count: importResults.success })}
                    </Text>
                    {importResults.products && importResults.products.length > 0 && (
                      <>
                        <Text size="sm" fw="500" mt="sm">
                          {t('inventory:importedProducts')}:
                        </Text>
                        <List size="sm" mt="xs">
                  ...
                  <>
                    <Alert
                      icon={<IconCheck size={16} />}
                      title={t('inventory:importCompleteWithIssues')}
                      color="green"
                    >
                      <Text size="sm" mb="xs">
                        {t('inventory:partialImport', {
                      ...
                      {importResults.products && importResults.products.length > 0 && (
                        <>
                          <Text size="sm" fw="500" mt="sm">
                            {t('inventory:successfullyImportedProducts')}:
                          </Text>
                          <List size="sm" mt="xs">
                            {importResults.products.map((product, index) => (
                              <List.Item
                    ...
                    {importResults.errors.length > 0 && (
                      <Alert
                        icon={<IconX size={16} />}
                        title={t('inventory:importErrors')}
                        color="red"
                      >
                        <Text size="sm" mb="xs">
                          {t('inventory:productsFailedValidation', {
                  ...
                  <Alert icon={<IconX size={16} />} title={t('inventory:importFailed')} color="red">
                    <Text size="sm" mb="xs">
                      {t('inventory:noProductsImported')}
                    </Text>
                    <Text size="sm" mt="xs" fw="500">
                      {t('inventory:validationErrors')}:
                    </Text>
                    <Stack gap="xs" mt="xs">
                      {importResults.errors.map((error, index) => (
                        <Box
      ...
      <Group justify="space-between" mt="xl">
        ...
        <Group>
          ...
          {active === 1 && <Button onClick={nextStep}>{t('common:next')}</Button>}
