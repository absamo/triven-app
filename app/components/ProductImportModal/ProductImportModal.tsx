import { Alert, Box, Button, Group, List, Modal, Progress, Stack, Stepper, Text, ThemeIcon } from '@mantine/core'
import { Dropzone } from '@mantine/dropzone'
import { IconAlertCircle, IconCheck, IconDownload, IconFileText, IconUpload, IconX } from '@tabler/icons-react'
                                ...
                                {importResults.errors.length === 0 && (!importResults.warnings || importResults.warnings.length === 0) ? (
                                    <Alert icon={<IconCheck size={16} />} title={t('inventory:importSuccessful')} color="green">
                                        ...
                                        {importResults.products && importResults.products.length > 0 && (
                                            <>
                                                <Text size="sm" fw="500" mt="sm">{t('inventory:importedProducts')}:</Text>
                                                <List size="sm" mt="xs">
                                    ...
                                    <>
                                        <Alert icon={<IconCheck size={16} />} title={t('inventory:importCompleteWithIssues')} color="green">
                                            ...
                                            {importResults.products && importResults.products.length > 0 && (
                                                <>
                                                    <Text size="sm" fw="500" mt="sm">{t('inventory:successfullyImportedProducts')}:</Text>
                                                    <List size="sm" mt="xs">
                                                        {importResults.products.map((product, index) => (
                                                            <List.Item key={index} icon={<ThemeIcon color="green" size={16} radius="xl"><IconCheck size={10} /></ThemeIcon>}>
                                                                {product.name}
                                        ...
                                        {importResults.errors.length > 0 && (
                                            <Alert icon={<IconX size={16} />} title={t('inventory:importErrors')} color="red">
                                                <Text size="sm" mb="xs">
                                                    {t('inventory:productsFailedValidation', { count: importResults.errors.length })}
                                                </Text>
                                                <Stack gap="xs">
