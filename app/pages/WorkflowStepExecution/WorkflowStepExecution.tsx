...
import { IconCheck, IconClock, IconInfoCircle, IconX } from '@tabler/icons-react'
...
export default function WorkflowStepExecutionPage() {
  ...
  const { workflowInstance, currentStepExecution, canApprove } =
    useLoaderData<WorkflowStepExecutionPageData>()
  ...
  return (
    <Container size="md" py="md">
      ...
      <Group justify="space-between" mb="xl">
        <div>
          <Title order={2}>Workflow Step Approval</Title>
          <Text c="dimmed" size="sm">
            {workflowInstance.workflowTemplate.name} -{' '}
            {formatEntityType(workflowInstance.entityType)} #{workflowInstance.entityId}
          </Text>
        </div>
        <Button variant="subtle" onClick={() => navigate('/workflow-instances')}>
          Back to Workflows
        </Button>
      </Group>
      ...
      <Card shadow="sm" padding="xl" radius="md" withBorder mb="xl">
        ...
        {currentStepExecution.timeoutAt && (
          <Alert icon={<IconClock size={16} />} color="orange" mb="md">
            <Text size="sm">
              This step will timeout on{' '}
              {formatLocalizedDate(currentStepExecution.timeoutAt, i18n.language, 'LLL')}
            </Text>
          </Alert>
        )}
