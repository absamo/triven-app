...
export default function WorkflowTemplatesPage() {
    ...
    const handleTemplateAction = async (data: {
        ...
        try {
            ...
            const actionVerb = data.action === 'create' ? t('workflows:messages.created', 'created') :
                data.action === 'update' ? t('workflows:messages.updated', 'updated') :
                    data.action === 'clone' ? t('workflows:messages.cloned', 'cloned') :
                        t('workflows:messages.processed', 'processed')

            notifications.show({
                title: t('common:success', 'Success'),
                message: t('workflows:messages.templateActionSuccess', 'Template {{action}} successfully!', { action: actionVerb }),
                color: 'green',
                autoClose: 3000,
            })
            ...
            notifications.show({
                ...
                message: t('workflows:messages.actionFailed', 'Failed to process the template action. Please try again.'),
                color: 'red',
            })
        }
    }
    ...
    const confirmDeleteTemplate = async () => {
        ...
        try {
            ...
            notifications.show({
                title: t('common:error', 'Error'),
                message: t('workflows:messages.deleteFailed', 'Failed to delete the template. Please try again.'),
                color: 'red',
            })
        }
    }
    ...
    const handleCloneTemplate = async (templateId: string) => {
        try {
            ...
            if (result.success && result.template) {
                notifications.show({
                    title: t('common:success', 'Success'),
                    message: t('workflows:messages.templateActionSuccess', 'Template {{action}} successfully!', { action: t('workflows:messages.cloned', 'cloned') }),
                    color: 'green',
                    autoClose: 3000,
                })
    ...
    const toggleTemplateStatus = async (templateId: string, isActive: boolean) => {
        try {
            ...
            notifications.show({
                title: t('common:error', 'Error'),
                message: t('workflows:messages.statusUpdateFailed', 'Failed to update template status. Please try again.'),
                color: 'red',
            })
        }
    }
    ...
    const getTriggerTypeColor = (triggerType: string) => {
        switch (triggerType) {
            ...
            case "purchase_order_threshold": return "blue"
            case "sales_order_create":
            case "sales_order_threshold": return "green"
            case "stock_adjustment_create": return "orange"
            case "transfer_order_create": return "purple"
            case "invoice_create": return "cyan"
            case "bill_create": return "indigo"
            case "customer_create": return "teal"
            case "supplier_create": return "lime"
            case "product_create": return "pink"
            case "low_stock_alert": return "red"
            case "high_value_transaction": return "yellow"
        ...
        <Container size="xl" py="md" w={"100%"}>
            ...
            <Grid gutter="lg" mb="xl">
                <Grid.Col span={{ base: 12, sm: 6, md: 4 }}>
                    <Card withBorder padding="lg" radius="md" style={{ backgroundColor: 'var(--mantine-color-green-light)', height: '120px', display: 'flex', flexDirection: 'column' }}>
                ...
                <Grid.Col span={{ base: 12, sm: 6, md: 4 }}>
                    <Card withBorder padding="lg" radius="md" style={{ backgroundColor: 'var(--mantine-color-red-light)', height: '120px', display: 'flex', flexDirection: 'column' }}>
                        <Group justify="space-between" align="flex-start" style={{ flex: 1 }}>
                            <div style={{ flex: 1 }}>
                                <Text size="xs" c="dimmed" fw={700} tt="uppercase" mb="xs">
                ...
                <Grid.Col span={{ base: 12, sm: 6, md: 4 }}>
                    <Card withBorder padding="lg" radius="md" style={{ backgroundColor: 'var(--mantine-color-yellow-light)', height: '120px', display: 'flex', flexDirection: 'column' }}>
                        <Group justify="space-between" align="flex-start" style={{ flex: 1 }}>
                            <div style={{ flex: 1 }}>
                                <Text size="xs" c="dimmed" fw={700} tt="uppercase" mb="xs">
            ...
            <Paper p="xl" withBorder mb="xl" radius="md" style={{ background: getFilterGradient() }}>
                <Group justify="space-between" mb="md">
                    <Group>
                        ...
                        <Title order={4} c={getTitleColor()}>{t('workflows:filters.title', 'Filter Templates')}</Title>
                    </Group>
                    <Button
                        variant="subtle"
                        size="xs"
                            ...
                            <Group justify="space-between" mb="md">
                                <div style={{ flex: 1 }}>
                                    <Group gap="sm" align="center" mb="xs">
                                        ...
                                        <div>
                                            <Text size="lg" fw={600} c={getCardTextColor()}>{template.name}</Text>
                                        </div>
                                    </Group>
                                    <Group gap="xs">
                                        <Badge
                                ...
                                <div onClick={(e) => e.stopPropagation()}>
                                    ...
                                    >
                                        ...
                                        <Menu.Item
                                            ...
                                            leftSection={template.isActive ? <IconX size={16} /> : <IconCheck size={16} />}
                                        >
                                            {template.isActive ? t('workflows:actions.deactivate', 'Deactivate') : t('workflows:actions.activate', 'Activate')}
                                        </Menu.Item>
                                        <Menu.Item
                                            color="red"
                                ...
                                <Group gap="xs">
                                    {template.steps.slice(0, 3).map((step, index) => (
                                        <Group key={step.id} gap="xs">
                                            <Badge
                                                variant="light"
                                                color={step.type === 'approval' ? 'green' : step.type === 'review' ? 'blue' : 'gray'}
                                                size="sm"
                                            >
                                                {step.name}
                                            </Badge>
                        ...
                        <div style={{ textAlign: 'center' }}>
                            ...
                            <Text c="dimmed" size="sm" mb="lg">
                                {(workflowTemplates || []).length === 0
                                    ? t('workflows:messages.noTemplatesDescription', "Create your first workflow template to streamline approval processes.")
                                    : t('workflows:messages.noMatchingTemplates', "No templates match the current filters. Try adjusting your search criteria.")
                                }
                            </Text>
                            {(workflowTemplates || []).length === 0 && (
                                <Button
            ...
            >
                <Text size="sm" mb="lg">
                    {t('workflows:messages.confirmDelete', 'Are you sure you want to delete this workflow template?')}
                </Text>
                {templateToDelete && (
