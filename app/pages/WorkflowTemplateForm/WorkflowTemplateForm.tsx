...
import { type IWorkflowStep, type IWorkflowTemplate } from "~/app/common/validations/workflowTemplateSchema"
...
interface WorkflowStep {
    ...
    name: string
    description?: string
    type: 'approval' | 'review' | 'notification' | 'data_validation' | 'automatic_action' | 'conditional_logic' | 'parallel_approval' | 'sequential_approval' | 'escalation' | 'integration'
...
export default function WorkflowTemplateFormPage() {
    ...
    const stepTypeOptions = [
        { value: "approval", label: t('workflows:stepTypes.approval', 'Approval'), icon: IconCheck, color: "green" },
        { value: "notification", label: t('workflows:stepTypes.notification', 'Notification'), icon: IconBell, color: "blue" },
        { value: "data_validation", label: t('workflows:stepTypes.data_validation', 'Data Validation'), icon: IconShieldCheck, color: "orange" },
        { value: "automatic_action", label: t('workflows:stepTypes.automatic_action', 'Automatic Action'), icon: IconRocket, color: "purple" },
        { value: "conditional_logic", label: t('workflows:stepTypes.conditional_logic', 'Conditional Logic'), icon: IconGitBranch, color: "cyan" },
        { value: "parallel_approval", label: t('workflows:stepTypes.parallel_approval', 'Parallel Approval'), icon: IconUsers, color: "teal" },
        { value: "sequential_approval", label: t('workflows:stepTypes.sequential_approval', 'Sequential Approval'), icon: IconChevronDown, color: "indigo" },
        { value: "escalation", label: t('workflows:stepTypes.escalation', 'Escalation'), icon: IconTrendingUp, color: "red" },
        { value: "integration", label: t('workflows:stepTypes.integration', 'Integration'), icon: IconPlug, color: "dark" },
    ]
    ...
    const assigneeTypeOptions = [
        ...
        { value: "department_head", label: t('workflows:assigneeTypes.department_head', 'Department Head') },
    ]
    ...
    const form = useForm<IWorkflowTemplate>({
        mode: 'uncontrolled',
        validate: {
            name: (value) => !value ? t('workflows:validation.templateNameRequired', 'Template name is required') : null,
            entityType: (value) => !value ? t('workflows:validation.entityTypeRequired', 'Entity type is required') : null,
            steps: (value) => !value || value.length === 0 ? t('workflows:validation.stepsRequired', 'At least one workflow step is required') : null,
        },
        initialValues: {
    ...
    useEffect(() => {
        if (!hasUserChanges && workflowTemplate) { // Only update if template exists (edit mode)
            form.setValues({
        ...
        if (fetcher.state === "idle" && fetcher.data && !loading) {
            ...
            if (responseData?.success) {
                ...
                notifications.show({
                    ...
                    message: responseData.error || t('workflows:messages.updateFailed', 'Failed to update the template. Please try again.'),
                    color: 'red',
                })
            }
        }
            ...
            if (step.id === stepId) {
                ...
                if (updates.assigneeId || updates.assigneeType) {
                    ...
                    if (assigneeType === 'user') {
                        ...
                        updatedStep.assigneeName = assigneeType === 'creator' ? 'Creator' :
                            assigneeType === 'manager' ? 'Manager' :
                                assigneeType === 'department_head' ? 'Department Head' : ''
                    }
                }
    ...
    const handleTemplateAction = async (data: {
        ...
        try {
            ...
            notifications.show({
                title: t('common:error', 'Error'),
                message: t('workflows:messages.updateFailed', 'Failed to update the template. Please try again.'),
                color: 'red',
            })
        }
    }
                ...
                <Group justify="space-between">
                    <div>
                        ...
                        <Text c="dimmed" size="sm">
                            {workflowTemplate
                                ? t('workflows:descriptions.editTemplate', 'Modify the workflow template configuration, steps, and settings.')
                                : t('workflows:descriptions.createTemplate', 'Create a new workflow template with steps and settings.')
                            }
                        </Text>
                    </div>
                </Group>
                        ...
                        <Grid gutter="md">
                            ...
                            <Grid.Col span={12}>
                                <Textarea
                                    ...
                                    placeholder={t('workflows:placeholders.description', 'Enter template description...')}
                            ...
                            <Grid.Col span={{ base: 12, sm: 6 }}>
                                <Select
                                    ...
                                    data={[
                                        ...
                                        { value: "purchase_order_create", label: t('workflows:triggerTypes.purchase_order_create', 'Purchase Order Created') },
                                        { value: "purchase_order_threshold", label: t('workflows:triggerTypes.purchase_order_threshold', 'Purchase Order Threshold') },
                                        { value: "sales_order_create", label: t('workflows:triggerTypes.sales_order_create', 'Sales Order Created') },
                                        { value: "sales_order_threshold", label: t('workflows:triggerTypes.sales_order_threshold', 'Sales Order Threshold') },
                                        { value: "stock_adjustment_create", label: t('workflows:triggerTypes.stock_adjustment_create', 'Stock Adjustment Created') },
                                        { value: "transfer_order_create", label: t('workflows:triggerTypes.transfer_order_create', 'Transfer Order Created') },
                                        { value: "customer_create", label: t('workflows:triggerTypes.customer_create', 'Customer Created') },
                                        { value: "high_value_transaction", label: t('workflows:triggerTypes.high_value_transaction', 'High Value Transaction') },
                                        { value: "custom_condition", label: t('workflows:triggerTypes.custom_condition', 'Custom Condition') },
                                    ]}
                            ...
                            <Grid.Col span={12}>
                                <Checkbox
                                    ...
                                    description={t('workflows:descriptions.isActive', 'Active templates can be used for new workflows')}
                                    ...
                                    {form.values.steps.map((step: IWorkflowStep, index: number) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <Text c="dimmed" size="sm" mt="sm">Loading drag functionality...</Text>
                                            </Card>
                                        )
                                    })}
                                </Stack>
                                                        ...
                                                        >
                                                            {(provided, snapshot) => (
                                                                <Card
                                                                    ...
                                                                    p="md"
                                                                    radius="md"
                                                                    className={`${classes.stepCard} ${snapshot.isDragging ? classes.stepCardDragging : ''
                                                                        }`}
                                                                    style={{
                                                                    ...
                                                                    <Grid gutter="md">
                                                                        <Grid.Col span={{ base: 12, sm: 6 }}>
                                                                            <TextInput
                                                                                ...
                                                                                placeholder={t('workflows:placeholders.stepName', 'Enter step name...')}
                                                                                value={step.name}
                                                                                onChange={(e) => {
                                                                        ...
                                                                        <Grid.Col span={{ base: 12, sm: 6 }}>
                                                                            <Select
                                                                                ...
                                                                                placeholder={t('workflows:placeholders.stepType', 'Select step type...')}
                                                                        ...
                                                                        <Grid.Col span={12}>
                                                                            <Textarea
                                                                                ...
                                                                                placeholder={t('workflows:placeholders.stepDescription', 'Enter step description...')}
                                                                        ...
                                                                        <Grid.Col span={{ base: 12, sm: 6 }}>
                                                                            <Select
                                                                                ...
                                                                                placeholder={t('workflows:placeholders.assigneeType', 'Select assignee type...')}
                                                                                data={assigneeTypeOptions}
                                                                                value={step.assigneeType}
                                                                                onChange={(value) => updateStep(step.id, { assigneeType: value as any, assigneeId: '', assigneeName: '' })}
                                                                            />
                                                                        </Grid.Col>
                                                                        {(step.assigneeType === 'user' || step.assigneeType === 'role') && (
                                                                            <Grid.Col span={{ base: 12, sm: 6 }}>
                                                                                <Select
                                                                                    label={step.assigneeType === 'user' ? t('workflows:fields.assigneeUser', 'Assignee User') : t('workflows:fields.assigneeRole', 'Assignee Role')}
                                                                                    placeholder={step.assigneeType === 'user' ? t('workflows:placeholders.assigneeUser', 'Select user...') : t('workflows:placeholders.assigneeRole', 'Select role...')}
                                                                                    data={getAssigneeOptions(step.assigneeType)}
                                                                                    value={step.assigneeId}
                                                                                    onChange={(value) => updateStep(step.id, { assigneeId: value || '' })}
                                                                                    searchable
                                                                                />
                                                                            </Grid.Col>
                                                                        )}
                                                                        <Grid.Col span={{ base: 12, sm: 6 }}>
                                                                            <NumberInput
                                                                                ...
                                                                                placeholder={t('workflows:placeholders.timeoutDays', 'Enter timeout in days...')}
                                                                        ...
                                                                        {step.type === 'conditional_logic' && (
                                                                            <Grid.Col span={12}>
                                                                                <JsonInput
                                                                                    label={t('workflows:fields.conditions', 'Conditions (JSON)')}
                                                                                    placeholder={t('workflows:placeholders.conditions', 'Enter conditions in JSON format...')}
