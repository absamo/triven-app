...
export default function BackorderForm({
    ...
    const form = useForm({
        ...
        initialValues: {
            ...
            originalOrderDate: backorder?.originalOrderDate ? new Date(backorder.originalOrderDate) : new Date(),
    ...
    const validateBackorderItems = (backorderItems: any[]) => {
        ...
        if (!backorderItems || backorderItems.length === 0) {
            return t('backorders:backorderItemRequired', 'Please add a backorder item before submitting the form');
        }
        ...
        if (invalidItems.length > 0) {
            return t('backorders:noProductSelectedError', 'Please select a product for all backorder items');
        }

        // Check if any backorder items have invalid quantities
        const invalidQuantityItems = backorderItems.filter(item => !item.orderedQuantity || item.orderedQuantity <= 0);
        if (invalidQuantityItems.length > 0) {
            return t('backorders:invalidQuantityError', 'Please enter a valid quantity for all backorder items');
        }
    ...
    return (
        <>
            <Title backTo="/backorders">
                {isEdit ? `${t('editBackorder')}` : t('createBackorder')}
            </Title>
                    ...
                    <Group justify="space-between" align="center">
                        <div>
                            ...
                            <Text size="xs" c="dimmed">
                                Order Date: {(backorder as any).salesOrder.orderDate ? formatLocalizedDate((backorder as any).salesOrder.orderDate, i18n.language, 'L') : 'N/A'} |
                ...
                <Grid>
                    ...
                    <Grid.Col span={12}>
                        <BackorderItemsForm
                            ...
                            siteId={form.values.siteId}
                            errors={errors}
                            alertComponent={form.errors.backorderItems && (
                                <Alert
                                    variant="light"
                                    color="red"
                                    icon={<IconExclamationCircle />}
                                ...
                                >
                                    <Text size="sm" c="red">{form.errors.backorderItems}</Text>
                                </Alert>
                            )}
                        />
                    </Grid.Col>
