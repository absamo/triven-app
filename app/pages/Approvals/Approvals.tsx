...
import { ApprovalActionModal, CardsGridSkeleton, FiltersSkeleton, TableSkeleton } from "~/app/components"
...
export default function ApprovalsPage() {
    ...
    const { approvalRequests, currentUser } = useLoaderData<{ approvalRequests: ApprovalRequest[], currentUser: CurrentUser }>()
    ...
    const [actionType, setActionType] = useState<'review' | 'add_review' | 'reject' | 'reopen' | null>(null)
    ...
    const handleDirectApproval = async (request: ApprovalRequest) => {
        if (!canUserActOnRequest(request)) {
            notifications.show({
                ...
                message: t('approvals:messages.noPermission', 'You do not have permission to act on this approval request.'),
                color: 'red',
            })
            return
        }
    ...
    const openModal = (request: ApprovalRequest, action: 'review' | 'add_review' | 'reject' | 'reopen') => {
    ...
    const handleApprovalAction = async (data: {
        ...
        try {
            ...
            const actionVerb = data.action === 'add_comment' ? t('approvals:messages.commentedOn', 'commented on') :
                data.status === 'approved' ? t('approvals:messages.approved', 'approved') :
                    data.status === 'rejected' ? t('approvals:messages.rejected', 'rejected') :
                        data.status === 'reopen' ? t('approvals:messages.reopened', 'reopened') :
                            t('approvals:messages.processed', 'processed')

            notifications.show({
                title: t('common:success', 'Success'),
                message: t('approvals:messages.requestActionSuccess', 'Request {{action}} successfully!', { action: actionVerb }),
                color: 'green',
            ...
            notifications.show({
                ...
                message: t('approvals:messages.actionFailed', 'Failed to process the approval action. Please try again.'),
                color: 'red',
            })
        }
    }
    ...
    if (fetcher.data && fetcher.data.error) {
        notifications.show({
            title: t('common:error', 'Error'),
            message: fetcher.data.error || t('approvals:messages.genericError', 'An error occurred while processing your request.'),
            color: 'red',
        })
    }
    ...
    const getWorkflowStepProgress = (request: ApprovalRequest) => {
        ...
        const hasStepExecution = request.stepExecution?.workflowStep?.stepNumber !== undefined && request.stepExecution?.workflowStep?.stepNumber !== null
        let currentStep = hasStepExecution ? request.stepExecution!.workflowStep.stepNumber : request.workflowInstance.currentStepNumber

        // If workflow is completed but currentStep is null, set currentStep to totalSteps so progress shows 100%
        if ((request.workflowInstance?.status === 'completed') && (currentStep === null || currentStep === undefined)) {
            ...
            <Container size="xl" py="md" w={"100%"}>
                ...
                <Grid gutter="lg" mb="xl">
                    ...
                    <Grid.Col span={{ base: 12, sm: 6, md: 3 }}>
                        <Card withBorder padding="lg" radius="md" style={{ backgroundColor: colorScheme === 'dark' ? '#25262b' : '#f8f9fa', height: '140px', display: 'flex', flexDirection: 'column' }}>
                    ...
                    {/* 2. In Review */}
                    <Grid.Col span={{ base: 12, sm: 6, md: 3 }}>
                        <Card withBorder padding="lg" radius="md" style={{ backgroundColor: colorScheme === 'dark' ? '#1a2332' : '#e7f5ff', height: '140px', display: 'flex', flexDirection: 'column' }}>
                            <Group justify="space-between" align="flex-start" style={{ flex: 1 }}>
                                <div style={{ flex: 1 }}>
                                    ...
                                    <Text size="xl" fw={700} c={getCardTextColor()} mb="xs">
                                        {filteredRequests.filter((r: ApprovalRequest) => r.status === 'in_review').length}
                                    </Text>
                                    <Text size="xs" c="dimmed">
                    ...
                    {/* 3. Approved */}
                    <Grid.Col span={{ base: 12, sm: 6, md: 3 }}>
                        <Card withBorder padding="lg" radius="md" style={{ backgroundColor: colorScheme === 'dark' ? '#1a4a1a' : '#e6fffa', height: '140px', display: 'flex', flexDirection: 'column' }}>
                            <Group justify="space-between" align="flex-start" style={{ flex: 1 }}>
                                <div style={{ flex: 1 }}>
                                    ...
                                    <Text size="xl" fw={700} c={getCardTextColor()} mb="xs">
                                        {filteredRequests.filter((r: ApprovalRequest) => r.status === 'approved').length}
                                    </Text>
                                    <Text size="xs" c="dimmed">
                    ...
                    {/* 4. Rejected */}
                    <Grid.Col span={{ base: 12, sm: 6, md: 3 }}>
                        <Card withBorder padding="lg" radius="md" style={{ backgroundColor: colorScheme === 'dark' ? '#4a1a1a' : '#fff5f5', height: '140px', display: 'flex', flexDirection: 'column' }}>
                            <Group justify="space-between" align="flex-start" style={{ flex: 1 }}>
                                <div style={{ flex: 1 }}>
                                    ...
                                    <Text size="xl" fw={700} c={getCardTextColor()} mb="xs">
                                        {filteredRequests.filter((r: ApprovalRequest) => r.status === 'rejected').length}
                                    </Text>
                                    <Text size="xs" c="dimmed">
                ...
                <Paper p="xl" withBorder mb="xl" radius="md" style={{ background: getFilterGradient() }}>
                    <Group justify="space-between" mb="md">
                        <Group>
                            ...
                            <Title order={4} c={getTitleColor()}>{t('approvals:filters.title', 'Filter Requests')}</Title>
                        </Group>
                        <Button
                            variant="subtle"
                            size="xs"
                            ...
                            >
                                ...
                                <Group justify="space-between" mb="md">
                                    <div style={{ flex: 1 }}>
                                        <Group gap="sm" align="center" mb="xs">
                                            <ThemeIcon
                                                ...
                                                radius="md"
                                                variant="light"
                                                color={request.entityType === 'purchase_order' ? 'blue' :
                                                    request.entityType === 'customer' ? 'green' :
                                                        request.entityType === 'stock_adjustment' ? 'orange' : 'gray'}
                                            >
                                                {request.entityType === 'purchase_order' ? <IconShoppingCart size={20} /> :
                                                    request.entityType === 'customer' ? <IconUser size={20} /> :
                                                        request.entityType === 'stock_adjustment' ? <IconTrendingUp size={20} /> :
                                                            <IconFileText size={20} />}
                                            </ThemeIcon>
                                            <div>
                                                <Text size="lg" fw={600} c={getCardTextColor()}>{request.title}</Text>
                                                <Text size="xs" c="dimmed">{formatEntityType(request.entityType)}</Text>
                                            </div>
                                        </Group>
                                        <Group gap="xs">
                                            <Badge
                                                ...
                                                size="sm"
                                            >
                                                {t(`approvals:statuses.${request.status}`, request.status.charAt(0).toUpperCase() + request.status.slice(1))}
                                            </Badge>
                                            {(() => {
                                    ...
                                    <Group gap="xs">
                                        <Badge
                                            ...
                                            variant="gradient"
                                            gradient={request.priority === 'Critical' ? { from: 'red', to: 'pink' } : undefined}
                                            size="lg"
                                        >
                                            {t(`approvals:priorities.${request.priority.toLowerCase()}`, request.priority)}
                                        </Badge>
                                    </Group>
                                </Group>
                                ...
                                <Grid gutter="md" mb="md">
                                    <Grid.Col span={3}>
                                        <Group gap="xs" align="center">
                                            ...
                                            <div>
                                                <Text size="xs" c="dimmed" fw={500}>{t('approvals:request.requestedDate', 'Requested date')}</Text>
                                    ...
                                    <Grid.Col span={3}>
                                        <Group gap="xs" align="center">
                                            ...
                                            <div>
                                                <Text size="xs" c="dimmed" fw={500}>{t('approvals:request.requestedBy')}</Text>
                                                <Text size="sm" fw={500} c={getCardTextColor()}>
                                                    {request.requestedByUser?.profile?.firstName || request.requestedByUser.email} {request.requestedByUser?.profile?.lastName || ''}
                                                </Text>
                                            </div>
                                        </Group>
                                    </Grid.Col>
                                    <Grid.Col span={3}>
                                        <Group gap="xs" align="center">
                                            ...
                                            <div>
                                                <Text size="xs" c="dimmed" fw={500}>{t('approvals:request.assignedTo')}</Text>
                                                <Text size="sm" fw={500} c={getCardTextColor()}>
                                    ...
                                    <Grid.Col span={3}>
                                        <Group gap="xs" align="center">
                                            ...
                                            <div>
                                                <Text size="xs" c="dimmed" fw={500}>{t('approvals:request.expiresOn', 'Expires on')}</Text>
                                                <Text size="sm" fw={500} c={getCardTextColor()}>
                                    ...
                                    <Group justify="space-between" align="center">
                                        <div>
                                            <Text size="sm" fw={500} c={getCardTextColor()} mb="xs">
                                                {t('approvals:entityDetails', '{{entityType}} Details', { entityType: formatEntityType(request.entityType) })}
                                            </Text>
                                            <Group gap="lg">
                                                {request.data.amount && (
                                                    <div>
                                                        <Text size="xs" c="dimmed" fw={500}>{t('approvals:modal.amount')}</Text>
                                                ...
                                                {request.data.items && Array.isArray(request.data.items) && (
                                                    <div>
                                                        <Text size="xs" c="dimmed" fw={500}>{t('approvals:modal.items')}</Text>
                                                        <Text size="sm" fw={500} c={getCardTextColor()}>
                                                            {t('approvals:itemsCount', '{{count}} products', { count: request.data.items.length })}
                                                        </Text>
                                                    </div>
                                                )}
                                                {request.data.products && Array.isArray(request.data.products) && (
                                                    <div>
                                                        <Text size="xs" c="dimmed" fw={500}>{t('approvals:modal.products')}</Text>
                                                        <Text size="sm" fw={500} c={getCardTextColor()}>
                                                            {t('approvals:productsCount', '{{count}} items', { count: request.data.products.length })}
                                                        </Text>
                                                    </div>
                                                )}
                                                {request.data.supplier && (
                                                    <div>
                                                        <Text size="xs" c="dimmed" fw={500}>{t('approvals:modal.supplier')}</Text>
                                                        <Text size="sm" fw={500} c={getCardTextColor()}>
                                                            {request.data.supplier}
                                                        </Text>
                                                    </div>
                                                ...
                                                {request.data.companyName && (
                                                    <div>
                                                        <Text size="xs" c="dimmed" fw={500}>{t('approvals:modal.company')}</Text>
                                                        <Text size="sm" fw={500} c={getCardTextColor()}>
                                                            {request.data.companyName}
                                                        </Text>
                                                    </div>
                                                ...
                                                {request.data.fromSite && (
                                                    <div>
                                                        <Text size="xs" c="dimmed" fw={500}>{t('approvals:modal.from')}</Text>
                                                        <Text size="sm" fw={500} c={getCardTextColor()}>
                                                            {request.data.fromSite}
                                                        </Text>
                                                    </div>
                                                ...
                                                {request.data.toSite && (
                                                    <div>
                                                        <Text size="xs" c="dimmed" fw={500}>{t('approvals:modal.to')}</Text>
                                                        <Text size="sm" fw={500} c={getCardTextColor()}>
                                                            {request.data.toSite}
                                                        </Text>
                                                    </div>
                                ...
                                <Group gap="xs" mb="md">
                                    ...
                                    <Text size="sm" c="dimmed">
                                        {(request.comments?.length || 0)} {(request.comments?.length || 0) === 1 ? t('approvals:request.comments') : t('approvals:request.commentsPlural')}
                                    </Text>
                                    {request.comments && request.comments.some((c: ApprovalComment) => !c.isInternal) && (
                                        <Badge size="xs" color="gray">{t('approvals:commentVisibility.public', 'Public')}</Badge>
                                    )}
                                    {request.comments && request.comments.some((c: ApprovalComment) => c.isInternal) && (
                                        <Badge size="xs" color="gray">{t('approvals:commentVisibility.internal', 'Internal')}</Badge>
                                    )}
                                </Group>
                                ...
                                {(request.status === "approved" || request.status === "rejected") && canUserActOnRequest(request) && (
                                    <Group justify="flex-end" gap="sm">
                                        <Button
                                            variant="light"
                                            size="sm"
                            ...
                            <div style={{ textAlign: 'center' }}>
                                ...
                                <Text c="dimmed" size="sm">
                                    {(approvalRequests || []).length === 0
                                        ? t('approvals:messages.noRequestsDescription', "There are no approval requests to display at this time.")
                                        : t('approvals:messages.noMatchingRequests', "No requests match the current filters. Try adjusting your search criteria.")
                                    }
                                </Text>
                            </div>
                        </Group>
