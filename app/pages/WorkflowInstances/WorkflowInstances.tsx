...
export default function WorkflowInstancesPage() {
    ...
    const filteredInstances = (workflowInstances || []).filter((instance: WorkflowInstance) => {
        if (searchQuery && !instance.workflowTemplate.name.toLowerCase().includes(searchQuery.toLowerCase()) &&
            !instance.entityId.toLowerCase().includes(searchQuery.toLowerCase())) return false
    ...
    const calculateProgress = (instance: WorkflowInstance) => {
        ...
        const completedSteps = instance.stepExecutions.filter(exec => exec.status === 'completed').length
                    ...
                    {filteredInstances.map((instance: WorkflowInstance) => (
                        ...
                        >
                            ...
                            <Group justify="space-between" mb="md">
                                <div>
                                    <Group gap="sm" align="center" mb="xs">
                                        ...
                                        <Text size="lg" fw={600}>{instance.workflowTemplate.name}</Text>
                                        <Badge
                                            variant="light"
                            ...
                            <Group mb="md">
                                ...
                                />
                                <Text size="xs" c="dimmed">
                                    {instance.stepExecutions.filter(exec => exec.status === 'completed').length} of {instance.workflowTemplate.steps.length} steps
                                </Text>
                            </Group>

                            {/* Timeline Preview */}
                            <Paper p="md" withBorder radius="md" style={{ backgroundColor: 'var(--mantine-color-default)' }}>
                                <Text size="sm" fw={500} mb="sm">Workflow Progress</Text>
                                <Timeline active={instance.currentStepNumber ? instance.currentStepNumber - 1 : instance.workflowTemplate.steps.length}>
                                    {instance.workflowTemplate.steps.map((step) => {
                                        const execution = instance.stepExecutions.find(exec => exec.workflowStep.stepNumber === step.stepNumber)
                                        ...
                                        return (
                                            <Timeline.Item
                                                key={step.stepNumber}
                                                bullet={isCompleted ? <IconCheck size={12} /> : isActive ? <IconClock size={12} /> : undefined}
                                                title={step.name}
                            ...
                            <Group justify="space-between" align="center" mt="md">
                                <Text size="sm" c="dimmed">
                                    Triggered by {instance.triggeredBy.profile?.firstName || instance.triggeredBy.email}
                                </Text>
                                <Group gap="sm">
